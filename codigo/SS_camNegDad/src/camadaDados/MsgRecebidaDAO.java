...
public class MsgRecebidaDAO implements Map<Integer, Mensagem> {
    ...
    public boolean containsValue(Object value) {throw new NullPointerException("Msg_Rec_containsValue não está implementado!");}    
    public Set<Map.Entry<Integer, Mensagem>> entrySet() {throw new NullPointerException("Msg_Rec_entrySet não está implementado!");}    
    public boolean equals(Object o) {throw new NullPointerException("Msg_Rec_equals não está implementado!");}
    
    public Mensagem get(Object key) {
        try {
            Integer chave = (Integer)key ;
            Mensagem res = null;
            String sql = "SELECT * FROM " + MENSAGEM_T + " WHERE m.receptor = ? AND m.id = ? " ;
            PreparedStatement stm = ConexaoBD.getConexao().prepareStatement(sql);            
            stm.setString(1, this.username) ; stm.setInt(2, chave) ;
            ResultSet rs = stm.executeQuery();
            if (rs.next())  {
