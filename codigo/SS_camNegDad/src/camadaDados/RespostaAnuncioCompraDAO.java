...
public class RespostaAnuncioCompraDAO implements Map<Integer, Anuncio> {
    ...
    public void clear () { 
        ...
        try {            
            String sql = "DELETE FROM " + RESPOSTA_A_T + " WHERE ra.anuncio = ?" ;
    ...
    public boolean containsKey(Object key) throws NullPointerException {
        ...
        try {
            ...
            String sql = "SELECT * FROM " + RESPOSTA_A_T + " WHERE ra.anuncio = ? AND ra.resposta = ?";
            PreparedStatement stm = ConexaoBD.getConexao().prepareStatement(sql);
    ...
    public boolean containsValue(Object value) {throw new NullPointerException("Resposta_Anuncio_containsValue não está implementado!");}    
    public Set<Map.Entry<Integer, Anuncio>> entrySet() {throw new NullPointerException("Resposta_Anuncio_entrySet não está implementado!");}    
    public boolean equals(Object o) {throw new NullPointerException("Resposta_Anuncio_equals não está implementado!");}
    ...
    public boolean isEmpty() {
        ...
        try {
            String sql = "SELECT * FROM " + RESPOSTA_A_T + " WHERE ra.anuncio = ?";
            PreparedStatement stm = ConexaoBD.getConexao().prepareStatement(sql);
            stm.setInt(1, this.codAnunc) ; 
            ResultSet rs = stm.executeQuery();
            return !rs.next();
    ...
    public Set<Integer> keySet() {
        ...
        try {
            ...
            String sql = "SELECT anuncio FROM " + RESPOSTA_A_T + " WHERE ra.anuncio = ?";
            PreparedStatement stm = ConexaoBD.getConexao().prepareStatement(sql);
            stm.setInt(1, this.codAnunc) ; 
            ResultSet rs = stm.executeQuery();         
            while(rs.next())
    ...
    public void putAll(Map<? extends Integer,? extends Anuncio> t) {throw new NullPointerException("Resposta_Anuncio_putAll não está implementado!");}
    ...
    public Anuncio remove (Object key) { 
        ...
        try {            
            Integer chave = (Integer)key ;
            Anuncio res = null ;
            String sql = "DELETE FROM " + RESPOSTA_A_T + " WHERE ra.anuncio = ? AND ra.resposta = ?" ;
            PreparedStatement stm = ConexaoBD.getConexao().prepareStatement(sql);
            stm.setInt(1, this.codAnunc) ;
            stm.setInt(2, chave) ;
            stm.execute();     
    ...
    public int size() {
        ...
        try {
            int i = 0;
            String sql = "SELECT resposta FROM " + RESPOSTA_A_T + " WHERE ra.anuncio = ?";
            PreparedStatement stm = ConexaoBD.getConexao().prepareStatement(sql);
            stm.setInt(1, this.codAnunc) ; 
            ResultSet rs = stm.executeQuery(); 
            for (; rs.next(); i++)
