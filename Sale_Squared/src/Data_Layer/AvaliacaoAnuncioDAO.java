...
public class AvaliacaoAnuncioDAO implements Map<Integer, Avaliacao> {
	...
	// construtor
	public AvaliacaoAnuncioDAO(int codAnuncArg) {
	...
	public void clear() {
		throw new NullPointerException(
				"Avaliacao_Anuncio_clear não está implementado!");
	}
	...
	public boolean containsKey(Object key) {
		...
		try {
			...
			String sql = "SELECT id FROM " + AVALIACAO_T
					+ " WHERE aval.anuncio = ? AND aval.id = ?";
	...
	public boolean containsValue(Object value) {
		throw new NullPointerException(
				"Avaliacao_Anuncio_containsValue não está implementado!");
	}

	public Set<Map.Entry<Integer, Avaliacao>> entrySet() {
		throw new NullPointerException(
				"Avaliavao_Anuncio_entrySet não está implementado!");
	}

	public boolean equals(Object o) {
		throw new NullPointerException(
				"Avaliacao_Anuncio_equals não está implementado!");
	}
	...
	public Avaliacao get(Object key) {
		...
		try {
			Avaliacao res = null;
			Integer chave = (Integer) key;
			String sql = "SELECT * FROM " + AVALIACAO_T
					+ " WHERE aval.anuncio = ? AND aval.id = ?";
			PreparedStatement stm = ConexaoBD.getConexao()
					.prepareStatement(sql);
			stm.setInt(1, this.codAnunc);
	...
	public Avaliacao put(Integer key, Avaliacao value) {
		...
		try {
			...
			stm.setInt(ID, key);
			stm.setString(COMENTARIO, value.getComentario());
			stm.setString(AVALIADOR, value.getAvaliador().getUsername());
	...
	public void putAll(Map<? extends Integer, ? extends Avaliacao> t) {
		throw new NullPointerException(
				"Avaliacao_putAll não está implementado!");
	}

	public Avaliacao remove(Object key) {
		throw new NullPointerException(
				"Avaliacao_remove não está implementado!");
	}
